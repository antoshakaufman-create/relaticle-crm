...
final class TaskResource extends Resource
{
    ...
    {
        return __('resources.task.label');
    }
        ...
        return $table
            ->columns([
                ...
                TextColumn::make('assignees.name')
                    ->label(__('resources.task.assignee'))
                    ->badge()
                    ->color('primary')
                    ->searchable()
            ...
            ->filters([
                Filter::make('assigned_to_me')
                    ->label(__('resources.task.assigned_to_me'))
                                ...
                                if ($assignees->isNotEmpty()) {
                                    $assignees->each(function (User $recipient) use ($record): void {
                                        ...
                                        if (! $notificationExists) {
                                            Notification::make()
                                                ->title(__('resources.common.new_task_assignment').': '.$record->title)
                                                ->actions([
                                                    Action::make('view')
                                                        ->button()
                                                        ->label(__('resources.common.view_task'))
