...
final class ValidateLeadsCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'leads:validate
                            {--status=pending : Статус лидов для валидации (pending, all)}
                            {--limit=100 : Максимальное количество лидов для валидации}';
     ...
     * @var string
     */
    protected $description = 'Валидация существующих лидов';
    ...
    public function handle(LeadValidationService $validationService): int
    {
        ...
        if ($leads->isEmpty()) {
            $this->warn('Лиды для валидации не найдены');

            return self::SUCCESS;
        }

        $this->info("Валидация {$leads->count()} лидов...");
        $this->newLine();
        ...
        foreach ($leads as $lead) {
            try {
                ...
                $errors++;
                $this->newLine();
                $this->error("Ошибка при валидации лида #{$lead->id}: ".$e->getMessage());
            }
        ...
        $this->info("Валидировано: {$validated}");
        if ($errors > 0) {
